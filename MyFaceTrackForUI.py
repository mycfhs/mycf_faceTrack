...
class MyFaceTrack(object):
    ...
    def getImg(self):
        ...
        elif self.faceExtractorMethod=='yolo' and self.yolo_init_done:
            from yolov5.utils.augmentations import letterbox
            import torch
            stride = int(self.yolo_model.stride.max())
            # names = self.yolo_model.module.names if hasattr(self.yolo_model, 'module') else self.yolo_model.names  # get class names
            img = letterbox(frame, (self.imgH, self.imgW), stride)[0]
            # Convert
            img = img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
            img = np.ascontiguousarray(img)
            img = torch.from_numpy(img).to(self.devive)
    ...
    def _yolo_init(self):
        ...
        self.devive = select_device(YOLO_DEVICE)

        self.yolo_model = attempt_load(YOLO_WEIGHT_PATH, map_location=self.devive)  # load FP32 model
        if YOLO_HALF_MODEL:
